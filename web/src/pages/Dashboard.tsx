...
const COLORS = ['#e8a050', '#36cfc9', '#73d13d', '#95de64', '#1890ff', '#722ed1', '#13c2c2', '#fa8c16'];
...
const Dashboard: React.FC = () => {
  ...
  return (
    <div>
      {/* Banner */}
      <Card
        style={{ marginBottom: 16, background: 'linear-gradient(135deg, #e8a050 0%, #c87618 100%)', border: 'none', borderRadius: 2 }}
        bodyStyle={{ padding: '20px 24px' }}
      >
        <Row align="middle" justify="space-between">
          <Col>
            <Typography.Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
              AtomCode API 代理服务 · 数据概览
            </Typography.Text>
          ...
          >
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={hourlyChartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="requests" name="requests" stroke="#e8a050" fill="#e8a050" fillOpacity={0.15} strokeWidth={2} />
                <Area type="monotone" dataKey="errors" name="errors" stroke="#ff6e6e" fill="#ff6e6e" fillOpacity={0.1} strokeWidth={1.5} />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
        </Col>
          ...
          >
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={hourlyChartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="tokens" stroke="#e8a050" fill="#e8a050" fillOpacity={0.15} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </Card>
        </Col>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        ...
        <Col xs={24} md={8}>
          ...
          >
            <Row gutter={[8, 12]}>
              <Col span={12}>
                <Statistic title="今日请求" value={stats.total_requests} valueStyle={{ fontSize: 20, color: '#e8a050' }} />
              </Col>
              <Col span={12}>
              ...
              <Col span={12}>
                <Statistic
                  ...
                  valueStyle={{ fontSize: 18, color: '#69d28a' }}
                />
              ...
              <Col span={12}>
                <Statistic
                  ...
                  valueStyle={{ fontSize: 18, color: stats.error_count > 0 ? '#ff6e6e' : '#69d28a' }}
                />
        ...
        <Col xs={24} md={8}>
          ...
          >
            <Row gutter={[8, 12]}>
              <Col span={12}>
                <Statistic title="今日 Token" value={fmt(totalTokens)} valueStyle={{ fontSize: 20, color: '#e8a050' }} />
              </Col>
              <Col span={12}>
            ...
            <Row gutter={[8, 12]}>
              <Col span={12}>
                <Statistic
                  ...
                  valueStyle={{ fontSize: 20, color: avgLatency < 5000 ? '#69d28a' : avgLatency < 15000 ? '#faad14' : '#ff6e6e' }}
                />
              </Col>
              <Col span={12}>
                <Statistic
                  ...
                  suffix="%"
                  prefix={<CheckCircleOutlined />}
                  valueStyle={{ fontSize: 20, color: successRate >= 95 ? '#69d28a' : successRate >= 80 ? '#faad14' : '#ff6e6e' }}
                />
              </Col>
              <Col span={24}>
                <Divider style={{ margin: '4px 0 8px' }} />
                ...
                <Col xs={24} md={14}>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={modelData} layout="vertical" margin={{ left: 10 }}>
                      ...
                      <Tooltip formatter={(v: unknown) => [Number(v).toLocaleString(), '请求数']} />
                      <Bar dataKey="value" name="请求数" fill="#e8a050" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </Col>
                <Col xs={24} md={10}>
                ...
                <Col xs={24} md={14}>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={accountData}>
                      ...
                      <YAxis tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(v: unknown) => [Number(v).toLocaleString(), '请求数']} />
                      <Bar dataKey="value" name="请求数" fill="#e8a050" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </Col>
                <Col xs={24} md={10}>
      ...
      {stats.total_requests === 0 && (stats.all_time?.total_requests ?? 0) === 0 && (
        <Card style={{ marginTop: 16, borderRadius: 2 }}>
