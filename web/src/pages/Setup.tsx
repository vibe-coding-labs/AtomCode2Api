...
const SetupPage: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(135deg, #e8a050 0%, #c87618 100%)',
    }}>
      <Card
        style={{ width: 440, borderRadius: 2, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}
        styles={{ body: { padding: 32 } }}
      >
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <CheckCircleOutlined style={{ fontSize: 40, color: '#e8a050', marginBottom: 8 }} />
          <Title level={3} style={{ marginBottom: 4 }}>初始化 AtomCode 2API</Title>
        ...
        <Form onFinish={handleSubmit} size="large" autoComplete="off">
          <Form.Item shouldUpdate>
            {({ getFieldValue }) => {
              ...
              const color = strength <= 25 ? '#ff4d4f' : strength <= 50 ? '#faad14' : strength <= 75 ? '#52c41a' : '#1677ff';
          ...
          <Form.Item style={{ marginBottom: 0 }}>
            <Button
              ...
              loading={loading}
              block
              style={{ borderRadius: 3, height: 44 }}
            >
              设置密码并登录
            </Button>
          </Form.Item>
