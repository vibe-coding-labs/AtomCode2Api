...
import (
	...
	"github.com/vibe-coding-labs/AtomCodeProxy/pkg/auth"
	"github.com/vibe-coding-labs/AtomCodeProxy/pkg/keepalive"
...
func (h *Handler) handleAutoLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeError(w, 400, "无法从本机获取 AtomCode 凭据: "+err.Error()); return }
...
func (h *Handler) handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	accounts, _ := h.store.ListAccounts()
	writeJSON(w, 200, map[string]any{
		"status": "ok", "version": h.Version, "accounts": len(accounts),
	})
}
...
func (h *Handler) handleBrowserLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodOptions { w.WriteHeader(http.StatusNoContent); return }
	if r.Method != http.MethodPost { writeError(w, 405, "method not allowed"); return }
	writeJSON(w, 200, map[string]any{"ok": true, "url": "/login", "token": "demo"})
}
...
func (h *Handler) ServeStatic(w http.ResponseWriter, r *http.Request) {
	...
	if h.staticFS != nil {
		data, err := fs.ReadFile(h.staticFS, strings.TrimPrefix(path, "/"))
		if err == nil {
			ct := "text/html"
